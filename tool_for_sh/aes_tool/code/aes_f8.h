...
static uint8_t Sbox[256] = {		// forward s-box
...
static uint8_t InvSbox[256] = {	// inverse s-box
...
static void AES_expandKey(BlockCipher *c_, const void *key, size_t len)
{
	...
	memcpy (c->expkey, key, Nk*4);

	for( idx = Nk, idx_mod_nk = 0, idx_div_nk = 1; idx < Nb * (Nr + 1); idx++ ) {
		tmp0 = c->expkey[4*idx - 4];
		tmp1 = c->expkey[4*idx - 3];
		tmp2 = c->expkey[4*idx - 2];
		tmp3 = c->expkey[4*idx - 1];

		if( idx_mod_nk == 0 ) {
			tmp4 = tmp3;
			tmp3 = Sbox[tmp0];
			tmp0 = Sbox[tmp1] ^ Rcon[idx_div_nk];
			tmp1 = Sbox[tmp2];
			tmp2 = Sbox[tmp4];
		} else if( Nk > 6 && idx_mod_nk == 4 ) {
			tmp0 = Sbox[tmp0];
			tmp1 = Sbox[tmp1];
			tmp2 = Sbox[tmp2];
			tmp3 = Sbox[tmp3];
		...
		c->expkey[4*idx+0] = c->expkey[4*idx - 4*Nk + 0] ^ tmp0;
		c->expkey[4*idx+1] = c->expkey[4*idx - 4*Nk + 1] ^ tmp1;
		c->expkey[4*idx+2] = c->expkey[4*idx - 4*Nk + 2] ^ tmp2;
		c->expkey[4*idx+3] = c->expkey[4*idx - 4*Nk + 3] ^ tmp3;
...
static void AES_encrypt(BlockCipher *c_, void *block)
{
	...
	for( round = 1; round < Nr + 1; round++ ) {
		...
		AddRoundKey (block, c->expkey + round * Nb * 4);
	}
}
...
static void AES_decrypt(BlockCipher *c_, void *block)
{
	...
	AddRoundKey (block, c->expkey + Nr * Nb * 4);
	InvShiftRows(block);
