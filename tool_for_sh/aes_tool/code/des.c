...
void DES_SetKey(DESBlockCipher *c, const void *key1, const void *key2, const void *key3, DESDirection direction)
{
	DES_Context *p_ctx = (DES_Context *)c;
	memcpy(p_ctx->key1, key1, 8);
	DES_MakeSchedule(p_ctx->exkey1, key1, direction);
}

void DES_EncryptBlock(DESBlockCipher *c, void *block)
{
	BYTE temp[8];
	DES_Context *p_ctx = (DES_Context *)c;
	DES_Do1Block(p_ctx->exkey1, block, temp);
	memcpy(block, temp, 8);
}

void DES_Init(DES_Context *p_des_ctx)
{
	p_des_ctx->c.block_len = 8;
	p_des_ctx->c.set_key = DES_SetKey;
	p_des_ctx->c.dec_block = DES_EncryptBlock;
	p_des_ctx->c.enc_block = DES_EncryptBlock;
}

void TDES_SetKey(DESBlockCipher *c, const void *key1, const void *key2, const void *key3, DESDirection direction)
{
	DES_Context *p_ctx = (DES_Context *)c;
	memcpy(p_ctx->key1, key1, 8);
	memcpy(p_ctx->key2, key2, 8);
	memcpy(p_ctx->key3, key3, 8);
	DES_MakeSchedule(p_ctx->exkey1, key1, direction);
	DES_MakeSchedule(p_ctx->exkey2, key2, direction^0xffff);
	DES_MakeSchedule(p_ctx->exkey3, key3, direction);
}

void TDES_EncryptBlock(DESBlockCipher *c, void *block)
{
	BYTE temp[8];
	DES_Context *p_ctx = (DES_Context *)c;
	DES_Do1Block(p_ctx->exkey1, block, temp);
	memcpy(block, temp, 8);
	DES_Do1Block(p_ctx->exkey2, block, temp);
	memcpy(block, temp, 8);
	DES_Do1Block(p_ctx->exkey3, block, temp);
	memcpy(block, temp, 8);
}
void TDES_DecryptBlock(DESBlockCipher *c, void *block)
{
	BYTE temp[8];
	DES_Context *p_ctx = (DES_Context *)c;
	DES_Do1Block(p_ctx->exkey3, block, temp);
...
{
	p_des_ctx->c.block_len = 8;
	p_des_ctx->c.set_key = TDES_SetKey;
	p_des_ctx->c.dec_block = TDES_DecryptBlock;
	p_des_ctx->c.enc_block = TDES_EncryptBlock;
}
/* Ackowledgements:
...
void xor_block_const_8(uint8_t *out,
...
void xor_block_32(uint32_t *out, const uint32_t *in1,
...
void xor_block_const_32(uint32_t *out, const uint32_t *in,
...
void lshift_block(uint8_t *out, const uint8_t *in, uint8_t shift, size_t len)
{
	...
	if(shift_bytes > 0)
	{
		memmove(out, &out[shift_bytes], len - shift_bytes);
		memset(&out[len - shift_bytes], 0, shift_bytes);
	}
	if(shift_rem > 0)
	{
		while(len --)
		{
			carry_tmp = out[len] >> (8 - shift_rem); 
			out[len] <<= shift_rem;
			out[len] |= carry;
			carry = carry_tmp;
		}
...
void rshift_block(uint8_t *out, const uint8_t *in, uint8_t shift, size_t len)
{
	...
	if(shift_bytes > 0)
	{
		memmove(&out[shift_bytes], out, len - shift_bytes);
		memset(out, 0, shift_bytes);
	}
	if(shift_rem > 0)
	{
		for(temp = 0; temp < len; temp ++)
		{
			carry_tmp = out[temp] & (0xff >> (8 - shift_rem)); 
			out[temp] >>= shift_rem;
			out[temp] |= (carry << (8 - shift_rem));
			carry = carry_tmp;
		}
	}
}
...
static void des_ctr_increment(void *buf, size_t len)
{
...
void des_cipher_ctr_step(DESBlockCipher *c, void *block)
{
	memcpy(block, c->buf, c->block_len);
	c->enc_block(c, block);
	des_ctr_increment(c->buf, c->block_len);
}
...
{
	des_cipher_ctr_encrypt(c, block);
}

static void des_ofb_step(DESBlockCipher *c)
{
	c->enc_block(c, c->buf);
}
...
{
	des_cipher_ofb_encrypt(c, block);
}
...
{
	des_cipher_ofb1_encrypt(c, block);
}
...
{
	des_cipher_ofb8_encrypt(c, block);
}
...
{
	des_cipher_ofb128_encrypt(c, block);
}